import javax.swing.*;
import java.awt.*;

public class SimulatorUI extends JFrame {
    private SimulatorController controller;
    private JTextArea codeArea;
    private JTextField wField;
    private JTextField pcField;
    private JLabel zeroFlagLabel;
    private JTable registerTable;

    public SimulatorUI(SimulatorController controller) {
        this.controller = controller;
        setTitle("PIC CPU Simulator");
        setSize(700, 500);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setLayout(new BorderLayout());

        // Top Panel: Controls
        JPanel controlPanel = new JPanel();
        JButton stepButton = new JButton("Step");
        JButton runButton = new JButton("Run");
        JButton resetButton = new JButton("Reset");
        controlPanel.add(stepButton);
        controlPanel.add(runButton);
        controlPanel.add(resetButton);
        add(controlPanel, BorderLayout.NORTH);

        // Center Panel: Code Editor & Registers Table
        JPanel centerPanel = new JPanel(new GridLayout(1, 2));
        
        codeArea = new JTextArea(10, 20);
        codeArea.setText("MOVLW 10\nMOVWF 20\nMOVLW 5\nADDWF 20\nSLEEP");
        centerPanel.add(new JScrollPane(codeArea));

        String[] columnNames = {"Register", "Value"};
        Object[][] data = new Object[16][2];
        for (int i = 0; i < 16; i++) {
            data[i][0] = "RAM [" + i + "]";
            data[i][1] = 0;
        }
        registerTable = new JTable(data, columnNames);
        centerPanel.add(new JScrollPane(registerTable));
        
        add(centerPanel, BorderLayout.CENTER);

        // Bottom Panel: Status Flags & W Register
        JPanel statusPanel = new JPanel();
        wField = new JTextField(5);
        wField.setEditable(false);
        pcField = new JTextField(5);
        pcField.setEditable(false);
        zeroFlagLabel = new JLabel("Zero: false");

        statusPanel.add(new JLabel("W:"));
        statusPanel.add(wField);
        statusPanel.add(new JLabel("PC:"));
        statusPanel.add(pcField);
        statusPanel.add(zeroFlagLabel);
        add(statusPanel, BorderLayout.SOUTH);

        // Button Action Listeners (Fixed syntax)
        stepButton.addActionListener(e -> {
            loadProgramFromEditor();
            controller.step();
            updateUIState();
        });

        runButton.addActionListener(e -> {
            loadProgramFromEditor();
            controller.run();
            updateUIState();
        });

        resetButton.addActionListener(e -> {
            controller.reset();
            updateUIState();
        });

        updateUIState();
    }

    private void loadProgramFromEditor() {
        String[] lines = codeArea.getText().split("\n");
        controller.loadProgram(lines);
    }

    private void updateUIState() {
        CPU cpu = controller.getCpu(); //[span_26](start_span)[span_26](end_span)
        wField.setText(String.valueOf(cpu.getW())); //[span_27](start_span)[span_27](end_span)
        pcField.setText(String.valueOf(cpu.getPC())); //[span_28](start_span)[span_28](end_span)
        zeroFlagLabel.setText("Zero: " + cpu.getZeroFlag()); //[span_29](start_span)[span_29](end_span)

        int[] regs = cpu.getRegisters(); //[span_30](start_span)[span_30](end_span)
        for (int i = 0; i < 16; i++) {
            registerTable.setValueAt(regs[i], i, 1);
        }
    }

    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            SimulatorController controller = new SimulatorController();
            new SimulatorUI(controller).setVisible(true);
        });
    }
}
