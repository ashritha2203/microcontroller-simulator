...
```text
CALL instruction runs
        ↓
Current return address is pushed onto the stack
        ↓
Processor jumps to the subroutine
        ↓
Subroutine executes
        ↓
...
Return address is popped from the stack
        ↓
Return address is loaded into the Program Counter
        ↓
